...
<ul>
    {if eq($User:warning.reason,'locked')}
	<li>{concat('A number of failed attempts have been made to login using this username!')|i18n( 'design/admin/user/login',, hash( '%siteaccess_name', $site_access.name ) )|wash}</li>
	<li>{'Your account has been locked. Please contact your administrator for assistance.'|i18n( 'design/admin/user/login',, hash( '%siteaccess_name', $site_access.name ) )|wash}</li>
    {else}
...
{if $User:needspasswordchange}
    {if eq($User:warning.reason,'first_time')}
	<li>{'For security reasons, you must change your password the first time you access %siteaccess_name now that secure login procedures are in place.'|i18n( 'design/admin/user/login',, hash( '%siteaccess_name', $site_access.name ) )|wash}</li>
    ...
    {elseif eq($User:warning.reason,'short')}
	<li>{concat('The password you have chosen is too short! Please select password with at least ', $User:password_min, ' characters.')|i18n( 'design/admin/user/login',, hash( '%siteaccess_name', $site_access.name ) )|wash}</li>
	<li>{'Please select a new password by completing the additional fields below.'|i18n( 'design/admin/user/login',, hash( '%siteaccess_name', $site_access.name ) )|wash}</li>
    {elseif eq($User:warning.reason,'dupe')}
	<li>{'The password you have chosen has been used by you in the past...please try again with a brand new password'|i18n( 'design/admin/user/login',, hash( '%siteaccess_name', $site_access.name ) )|wash}</li>
	<li>{'Please select a new password by completing the additional fields below.'|i18n( 'design/admin/user/login',, hash( '%siteaccess_name', $site_access.name ) )|wash}</li>
    {elseif eq($User:warning.reason,'nomatch')}
	<li>{'The new password and confirmation did not match! please try again!'|i18n( 'design/admin/user/login',, hash( '%siteaccess_name', $site_access.name ) )|wash}</li>
	<li>{'Please select a new password by completing the additional fields below.'|i18n( 'design/admin/user/login',, hash( '%siteaccess_name', $site_access.name ) )|wash}</li>
    {else}
